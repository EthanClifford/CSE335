...
* indices.
*
* \param currFrame The frame that the pointer is currently at
*/
...
void CAnimChannel::InsertKeyframe(std::shared_ptr<Keyframe> keyframe)
{
    ...
    if (mKeyframe1 < 0)
    {
        // There is no first keyframe.  This means we are 
        // before any keyframes or there are no keyframes.
    ...
    else
    {
        ...
        if (mKeyframe2 < 0)
        {
            // There is no second keyframe.  If we are after the 
            // current frame, we append.  If we are on the current frame,
            // we replace
...
/** Removes a keyframe if the pointer is at the same frame as the keyframe
*/
void CAnimChannel::RemoveKeyframe()
{
    if (mKeyframe1 >= 0)
    {
        int currFrame = mTimeline->GetCurrentFrame();
        if (mKeyframes[mKeyframe1]->GetFrame() == currFrame)
        {
