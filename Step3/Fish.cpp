#include "pch.h"
#include <string>
#include "Fish.h"
#include "Aquarium.h"

/// Maximum speed in the X direction in
/// pixels per second
const double MaxSpeedX = 50;

/// Maximum speed in the Y direction in
/// pixels per second
const double MaxSpeedY = 50;

/**
 * Constructor
 * \param aquarium The aquarium we are working in 
 * \param filename The file name that we are writing to
 */
CFish::CFish(CAquarium *aquarium, const std::wstring &filename) : CItem(aquarium, filename)
{
	mSpeedX = ((double)rand() / RAND_MAX) * MaxSpeedX;
	mSpeedY = ((double)rand() / RAND_MAX) * MaxSpeedY;
}

/**
 * Handle updates in time of our fish
 *
 * This is called before we draw and allows us to move our fish.
 * We add our speed times the amount of time that has elapsed.
 * \param elapsed Time elapsed since the last class call
 */
void CFish::Update(double elapsed)
{
    SetLocation(GetX() + mSpeedX * elapsed, GetY() + mSpeedY * elapsed);

    int numPixelsToEdge = 10;
    if (mSpeedX > 0 && GetX() >= GetAquarium()->GetWidth() - numPixelsToEdge - GetWidth()/2)
    {
        mSpeedX = -mSpeedX;
        SetMirror(mSpeedX < 0);
    }
    else if (mSpeedX < 0 && GetX() <= numPixelsToEdge + GetWidth()/2)
    {
        mSpeedX = -mSpeedX;
        SetMirror(mSpeedX < 0);
    }
    else if (mSpeedY > 0 && GetY() >= GetAquarium()->GetHeight() - numPixelsToEdge - GetHeight() / 2)
    {
        mSpeedY = -mSpeedY;
    }
    else if (mSpeedY < 0 && GetY() <= numPixelsToEdge + GetHeight() / 2)
    {
        mSpeedY = -mSpeedY;
    }
}

/**
 * Save this item to an XML node
 * \param node The node we are going to be a child of
 * \return A pointer to the node that we added to the XML file
 */
std::shared_ptr<xmlnode::CXmlNode> CFish::XmlSave(const std::shared_ptr<xmlnode::CXmlNode>& node)
{
    auto itemNode = CItem::XmlSave(node);

    itemNode->SetAttribute(L"speedx", mSpeedX);
    itemNode->SetAttribute(L"speedy", mSpeedY);

    return itemNode;
}

/**
 * Load the attrubutes for an fish item.
 *
 * \param node The Xml node we are loading the item from
 *
 */
void CFish::XmlLoad(const std::shared_ptr<xmlnode::CXmlNode>& node)
{
    CItem::XmlLoad(node);
    mSpeedX = node->GetAttributeDoubleValue(L"speedx", 0);
    mSpeedY = node->GetAttributeDoubleValue(L"speedy", 0);
    if (mSpeedX < 0)
    {
        SetMirror(mSpeedX < 0);
    }
}
