...
using namespace std;
  ...
  * \param filename The file that we are reading the information from
  */
...
bool CItem::HitTest(int x, int y)
{
    ...
    if (format == PixelFormat32bppARGB || format == PixelFormat32bppPARGB)
    {
        ...
        // transperency.  If so, we should check to see if we clicked on a pixel where alpha is not zero, meaning
...
/**
 * Load the attrubutes for an item node.
 *
